...
double  CSecurity::getrealbuycost(int namount)
{
	...
	while (nQtyLeft>0)
	{
		...
		if (dAskPrice == this->m_DepthMarketData.marketinfor.dHighLimited)  //为涨停价时， 则结束计算冲击  默认之后冲击为0
		{
		...
		if (i == this->m_DepthMarketData.nMarketdepth) //超过行情深度时  
		{
...
double  CSecurity::getrealsellgain(int namount)
{
	...
	while (nQtyLeft>0)
	{
		...
		if (dBidPrice == this->m_DepthMarketData.marketinfor.dLowLimited)
		{
